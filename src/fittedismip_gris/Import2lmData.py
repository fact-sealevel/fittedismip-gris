...
# EM: I think we want to delete directory arg and hardcode it in sspfile?
# Or add a directory arg to preprocess + cli so that it can actually be set...
def Import2lmData(
    ...
    year_idx = np.flatnonzero(
        np.logical_and(years >= refyear_start, years <= refyear_end)
    )
    ...
    if scenario_test:
        ...
        window_years_idx = np.flatnonzero(
            np.logical_and(years >= twinyear_start, years <= twinyear_end)
        )

        # Get the sample indices that match the filter
        samps_max = np.nanmax(temp_samps[window_years_idx, :], axis=0)
        match_idx = np.logical_and(
            samps_max >= temp_target - temp_target_window,
            samps_max <= temp_target + temp_target_window,
        )
...
def Filter2lmData(tlm_dict, filter_years=None, tmin=-np.inf, tmax=np.inf):
    ...
    if not np.any(match_idx):
        raise Exception(
            "Cannot find samples that match the target window. min = {}, max = {}".format(
                tmin, tmax
            )
        )
